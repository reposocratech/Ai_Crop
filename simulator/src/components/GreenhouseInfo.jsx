    ...
    axios
      ...
      .then((res) => {
        ...
        if (res.data.resultParameters.length != 0) {
          ...
          setErrorParameters(
            "Este invernadero no tiene parámetros establecidos"
          );
          setParameters();
        }

        !res.data.resultGreenhouse[0] &&
          setMessageError(
            `No existe un invernadero con id ${""}. Intenta nuevamente`
          );
      })
      .catch((err) => {
        console.log(err);
    ...
    return (
      <div className="greenhouseInfo">
        <div className="cont_card_greenhouse">
          ...
          <div className="img_greenhouse">
            <img src="/assets/greenhouse.png" />
          </div>
              ...
              <tbody>
                {parameters?.map((elem, index) => {
                  return (
                    <tr key={index}>
                      <td>
                        {elem.measurement_type_name} ({elem.unit})
                      </td>
                      <td>{elem.min}</td>
                      <td>{elem.max}</td>
                    </tr>
