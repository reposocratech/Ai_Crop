...
export const UpdateCropModal = ({
  ...
  const handleSubmit = () => {
    if (
      editCropInfo?.crop_name &&
      editCropInfo?.crop_plant_variety &&
      editCropInfo?.crop_duration &&
      editCropInfo?.crop_size
    ) {
      axios
        .put(
          `http://localhost:4000/Crop/editCrop/${selectedCrop}`,
          editCropInfo
        )
        .then((res) => {
          setErrorMessage("");
      ...
      <Modal.Body className="divMasterCrop">
        <section className="d-flex secPpal ">
          ...
          <div className="botoneraCrops">
            {editCropInfo?.is_active === 1 && (
              <button className="botonCrops" onClick={handleSubmit}>
                Editar
              </button>
            )}

            <button className="botonCrops" onClick={handleSubmit2}>
              Eliminar
            </button>
          </div>
