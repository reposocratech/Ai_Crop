...
export const AllGreenhouses = () => {
  ...
  useEffect(() => {
    if (user) {
      axios
        .get(
          `http://localhost:4000/greenhouse/getAllGreenhouses/${user.user_id}`
        )
        .then((res) => {
    ...
    <div className="cont_greenhouses">
      ...
      <header className="header_greenhouses">
        <section className="title_row">
          ...
          <article className="input_sect"></article>
        </section>
        {greenhousesInfo && (
      ...
      <main>
        {user?.user_type === 2 && (
          <>
            ...
            <section className="greenhouse_list">
              {greenhousesInfo &&
                greenhousesInfo.resultOwner.map((elem, index) => {
                  return (
                    <div key={index}>
                      <OwnerCard elem={elem} />
                    </div>
        ...
        <section className="greenhouse_list">
          {greenhousesInfo &&
            greenhousesInfo.resultCollaborator.map((elem, index) => {
              return (
                <div key={index}>
